...
class AUCBanditMetaTechnique(MetaSearchTechnique):
    ...
    @classmethod
    def generate_technique(cls, manipulator=None, num_techniques=5, retry_count=3, generator_weight=10, *args,
                           **kwargs):
        """
        ...
        for i in range(num_techniques):
            for j in range(retry_count):
                ...
                if random.random() < old_div(float(len(basetechniques)),
                                             (len(basetechniques) + generator_weight * len(generators))):
